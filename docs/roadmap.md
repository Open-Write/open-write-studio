...
Hard-excluded paths: `exports/`, `.open-write/`.
...
2. **Snapshot before write** — every replace operation writes shadow copies of all files about to be modified to `.open-write/snapshots/global-replace/<ISO-timestamp>/` preserving relative paths, plus a `manifest.json` recording the search term, replacement string, match modes, and the list of files touched. Precursor to the Proposed "Local snapshot trail" feature.
...
Keep a per-save shadow copy of every chapter inside `.open-write/snapshots/<chapter-stem>/<ISO-timestamp>.md`, capped at the last N saves per chapter (default 50). The `.open-write/` directory is already cache, and users can exclude it from cloud sync. Provides a recovery path even when external forces (cloud sync rollback, antivirus restore, disk corruption) eat a chapter file. A simple "Recover from snapshot" UI in the chapter context menu lets the writer pick a prior version. Companion to the cloud-sync detection above.
