...
const HAS_DONATED_KEY        = "open-write.donation.hasDonated";
const APP_OPEN_COUNT_KEY     = "open-write.donation.appOpenCount";
const NEXT_PROMPT_AT_KEY     = "open-write.donation.nextPromptAt";
const DISMISS_TS_KEY         = "open-write.donation.lastDismissedAt";
...
const SESSION_INCREMENTED_KEY = "open-write.donation.sessionIncremented";
...
export function useDonationState(): UseDonationStateResult {
  ...
  useEffect(() => {
    ...
    // Donation prompts are disabled in the Open-Write build. The donor-flag
    // plumbing (hasDonated + the About badge) is retained so the hook's
    // consumers keep type-checking, but no fundraising nudge ever fires.
    // shouldShowPrompt stays at its initial false value.
  }, []);
