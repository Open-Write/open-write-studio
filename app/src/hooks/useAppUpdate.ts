...
export function useAppUpdate(): UseAppUpdateResult {
  ...
  const runCheck = useCallback(async (): Promise<AvailableUpdate | null> => {
    ...
    // Auto-update is disabled for the Open-Write build. The updater endpoint
    // and signing key are not configured (see tauri.conf.json), so we never
    // contact any release server. The hook reports "up-to-date" so the UI
    // surfaces nothing. Re-enable here once an Open-Write release feed and a
    // fresh minisign key exist, and re-add the updater plugin in tauri.conf.json.
