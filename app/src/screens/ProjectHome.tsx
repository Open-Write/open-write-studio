...
const STORY_TYPE_OPTIONS: {
  ...
  { value: "screenplay",     icon: Clapperboard,  range: "90 to 120 pages",     hint: "Feature film script. Fountain format, three-act structure." },
  { value: "tv_pilot",       icon: Tv,            range: "30 to 60 pages",      hint: "TV episode script. Season arc and episode structure." },
];
...
export function ProjectHome({ onProjectOpen }: ProjectHomeProps) {
  ...
  // under the configured vault root (default ~/Documents/Open-Write) using
  ...
  async function handleOpen() {
    ...
    try {
      ...
      if (data.kind === "project") {
        ...
        throw new Error("This folder doesn't look like an Open-Write project or series.");
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "An unexpected error occurred.");
    } finally {
          ...
          {error && (
            <div className="mt-6 rounded border border-red-800 bg-red-950/40 p-3">
              ...
              <p className="mt-1 text-xs text-red-400">
                Make sure the Open-Write backend is running on port 8000.
              </p>
            </div>
          )}
        </section>
