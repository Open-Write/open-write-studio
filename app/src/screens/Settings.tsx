...
// =========================================
// A scrollable modal overlay for configuring Open-Write's AI behaviour,
...
export function Settings({ onClose }: SettingsProps) {
  ...
  // The backend resolves an empty string to ~/Documents/Open-Write, so we
  ...
  // Multi-provider config + per-role pipeline models (Open-Write A/B).
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        // Multi-provider config + per-role pipeline models.
        setProviders(data.providers ?? []);
        setWriterModel(data.writer_model ?? "");
        setCriticModel(data.critic_model ?? "");
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Multi-provider config. Send the full list; the backend preserves masked
      // keys. New keys typed into providerKeyDrafts are layered on top.
      payload.providers = providers.map(p => ({
        id: p.id,
        label: p.label,
        base_url: p.base_url,
        models: p.models,
        api_key: providerKeyDrafts[p.id] ?? "",
      }));
      payload.writer_model = writerModel;
      payload.critic_model = criticModel;
  ...
  // All qualified "<provider>/<model>" options, for the role pickers' datalist.
  const qualifiedOptions: string[] = providers.flatMap(p =>
    (p.models ?? []).map(m => `${p.id}/${m}`)
  );

  async function handleProviderTest(id: string) {
    // Persist first so the backend sees the latest key/url, then probe.
    await handleSave();
    setProviderTestStatus(prev => ({ ...prev, [id]: null }));
    try {
      ...
      setProviderTestStatus(prev => ({
        ...prev,
        [id]: data.ok
          ? { ok: true, message: `Connected. ${data.model_count ?? 0} models.` }
          : { ok: false, message: data.error ?? "Connection failed." },
      }));
    } catch {
      setProviderTestStatus(prev => ({ ...prev, [id]: { ok: false, message: "Could not reach the backend." } }));
    }
  }
          ...
          {loading ? (
            ...
            <div className="space-y-8">
              ...
              <section>
                      ...
                      <div key={p.id} className="rounded border border-border bg-bg-surface p-3">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          <input
                            ...
                            placeholder="https://... (OpenAI-compatible root)"
                          ...
                          <input
                            type="password"
                            value={providerKeyDrafts[p.id] ?? ""}
                            onChange={e => setProviderKeyDrafts(prev => ({ ...prev, [p.id]: e.target.value }))}
                            placeholder={p.api_key_set ? `Current: ${p.api_key} -- enter new to replace` : "API key"}
                            className="w-full rounded border border-border bg-bg-base px-2 py-1.5 text-xs text-text-primary placeholder-faint outline-none focus:border-indigo-500"
                          />
                          <input
                            type="text"
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          <button
                            ...
                            disabled={saving}
                            className="flex items-center gap-1.5 rounded border border-border px-2.5 py-1 text-[11px] text-text-muted transition-colors hover:border-indigo-500 hover:text-text-primary disabled:opacity-50"
                            type="button"
                          >
                            Test {p.label}
                          </button>
                          {providerTestStatus[p.id] && (
                            <span className={`text-[11px] ${providerTestStatus[p.id]!.ok ? "text-emerald-400" : "text-red-400"}`}>
                              {providerTestStatus[p.id]!.ok ? <CheckCircle size={11} className="inline mr-1" /> : <XCircle size={11} className="inline mr-1" />}
                ...
                {/* ── Pipeline role models (Open-Write A/B) ───────────────── */}
                <div className="mb-5">
                  <label className="mb-1 block text-xs font-medium text-text-primary">
                    Pipeline Models (Autonomous Pipeline A/B)
                  </label>
                  <p className="mb-3 text-xs text-faint">
                    The autonomous pipeline can use different models for writing vs
                    critiquing (attacks self-recognition bias). Pick a qualified
                    "&lt;provider&gt;/&lt;model&gt;" for each role. Leave blank to use
                    the default model for both.
                  </p>
                  <datalist id="qualified-model-options">
                    {qualifiedOptions.map(opt => <option key={opt} value={opt} />)}
                  </datalist>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <span className="mb-1 block text-[11px] text-text-muted">Writer (author phases)</span>
                      <input
                        type="text"
                        list="qualified-model-options"
                        value={writerModel}
                        onChange={e => setWriterModel(e.target.value)}
                        placeholder="e.g. glm/glm-4.6"
                        className="w-full rounded border border-border bg-bg-base px-2 py-1.5 text-xs text-text-primary placeholder-faint outline-none focus:border-indigo-500"
                      />
                    </div>
                    <div>
                      <span className="mb-1 block text-[11px] text-text-muted">Critic (critic/editorial)</span>
                      <input
                        type="text"
                        list="qualified-model-options"
                        value={criticModel}
                        onChange={e => setCriticModel(e.target.value)}
                        placeholder="e.g. mimo/mimo-7b"
                        className="w-full rounded border border-border bg-bg-base px-2 py-1.5 text-xs text-text-primary placeholder-faint outline-none focus:border-indigo-500"
                      />
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      placeholder="C:\\Users\\You\\Documents\\Open-Write"
                    ...
                    <button
                      onClick={async () => {
                        const picked = await openDialog({
                          ...
                          title: "Choose a parent folder for new Open-Write projects",
                        });
                  ...
                  <p className="mt-2 text-xs text-faint">
                    ...
                    (<code className="text-indigo-400">~/Documents/Open-Write</code>).
                  </p>
                </div>
              </section>
