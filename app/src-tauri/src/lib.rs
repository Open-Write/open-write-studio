...
fn kill_sidecar_sync() {
    let _ = std::process::Command::new("taskkill")
        .args(["/F", "/IM", "open-write-backend.exe"])
        .output();
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        // updater plugin: DISABLED — no release feed or signing key configured yet.
        // Re-enable by uncommenting the line below AND restoring the plugins.updater
        // block in tauri.conf.json with a valid pubkey + endpoint.
        ...
        .setup(|_app| {
            ...
            #[cfg(not(debug_assertions))]
            {
                ...
                match _app.shell().sidecar("open-write-backend") {
                    Ok(cmd) => {
                        match cmd.spawn() {
                            Ok((mut rx, child)) => {
                    ...
                    Err(e) => {
                        eprintln!("Sidecar 'open-write-backend' not found: {e}");
                    }
                }
            }
            Ok(())
