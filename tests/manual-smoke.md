...
(e.g. `~/Documents/Open-Write/smoke-<date>/`) so each scenario
...
being held by `open-write-backend.exe` after the app window is gone.
...
- `exports/`
- `.open-write/` — cache; `app.db` will appear here after the first
  save event
