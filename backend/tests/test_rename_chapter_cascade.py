...
# =====================================
# Tests for the file-renaming chapter rename.
...
#
# NOTE: Open-Write uses "Chapter One" naming: 01-chapter-one.md
...
def test_rename_updates_heading_and_filename(client: TestClient, tmp_path: Path):
    ...
    res = _rename(client, root, "01-chapter-one.md", "The Beginning of X")
    assert res.status_code == 200
    data = res.json()

    assert data["old_filename"] == "01-chapter-one.md"
    ...
    assert not (manuscript / "01-chapter-one.md").exists()
...
def test_same_slug_is_heading_only_no_op(client: TestClient, tmp_path: Path):
    # "Chapter One" -> "CHAPTER ONE" slugs identically; the file must be left
    # alone and only the heading rewritten.
    root = _create_project(client, tmp_path)

    res = _rename(client, root, "01-chapter-one.md", "CHAPTER ONE")
    assert res.status_code == 200
    data = res.json()
    assert data["filename"] == "01-chapter-one.md"
    assert data["old_filename"] == "01-chapter-one.md"

    content = (Path(root) / "manuscript" / "01-chapter-one.md").read_text(encoding="utf-8")
    assert content.startswith("# CHAPTER ONE")
...
def test_collision_returns_409(client: TestClient, tmp_path: Path):
    ...
    res = _rename(client, root, "01-chapter-one.md", "The Storm")
    assert res.status_code == 409


def test_empty_title_is_400(client: TestClient, tmp_path: Path):
    root = _create_project(client, tmp_path)
    res = _rename(client, root, "01-chapter-one.md", "   ")
...
def test_cascade_moves_summary_and_scene_folder(client: TestClient, tmp_path: Path):
    ...
    (summaries / "chapters" / "01-chapter-one.md").write_text("Summary body\n", encoding="utf-8")
    scene_dir = summaries / "scenes" / "01-chapter-one"
    scene_dir.mkdir(parents=True)
    (scene_dir / "scene-01.md").write_text("# Opening\n\nScene summary\n", encoding="utf-8")

    res = _rename(client, root, "01-chapter-one.md", "Landfall")
    assert res.status_code == 200
    data = res.json()
    ...
    assert not (summaries / "chapters" / "01-chapter-one.md").exists()
    assert (summaries / "scenes" / "01-landfall" / "scene-01.md").is_file()
    assert not (summaries / "scenes" / "01-chapter-one").exists()
...
def test_rename_keeps_act_membership_and_position(client: TestClient, tmp_path: Path):
    ...
    client.put("/api/structure", json={
        "folder_path": root,
        "acts": [{"title": "Act I", "chapters": ["01-chapter-one.md", "02-second.md"]}],
        "unassigned": [],
    })
...
async def test_rename_migrates_progress_relpaths(client: TestClient, tmp_path: Path):
    ...
    await record_save_event(
        root, "manuscript/01-chapter-one.md",
