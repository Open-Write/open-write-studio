...
#   5. Normalize the response (whitespace; em dashes preserved)
...
class RunAssistantResponse(BaseModel):
    ...
    notes: list[str]
    model_used: str
    had_em_dashes: bool   # True if the model emitted em/en dashes (informational; not stripped)
...
async def run_assistant(request: RunAssistantRequest):
    ...
    The response includes had_em_dashes=True if the model emitted em/en dashes.
    These are no longer stripped (Open-Write advisory policy -- density is
    checked by the em_dash lint + naturalism critic). The flag is kept as an
    ...
    from app.ai.providers import resolve as _resolve_provider
    settings = load_settings()
    model_id = request.model_id or settings.get("default_model", "openai/gpt-4o-mini")
    try:
        _resolved = _resolve_provider(model_id)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    if not _resolved.is_configured:
        raise HTTPException(
            status_code=400,
            detail=(f"The '{_resolved.label}' provider isn't configured. "
                    f"Add its base URL and API key in Settings."),
        )
    api_key = _resolved.api_key
    base_url = _resolved.base_url
    model_id = _resolved.model_name
...
def _resolve_model_and_key(model_id_override: str | None) -> tuple[str, str, str]:
    """
    Load settings, validate the model's provider is configured (base_url +
    api_key present), and resolve (api_key, model_name, base_url) for the
    OpenAI-compatible client. Raises HTTPException 400 if the provider isn't
    configured. Helper shared by all Phase 4 generation endpoints.
    """
    from app.ai.providers import resolve
    model_id = model_id_override or load_settings().get("default_model", "openai/gpt-4o-mini")
    try:
...
async def generate_usage_preview(request: GenerateUsagePreviewRequest):
    ...
    try:
        result = await run_completion(api_key=api_key, model_id=model_id, base_url=base_url,
...
async def generate_chapter_summary(request: GenerateChapterSummaryRequest):
    ...
    Pipeline:
      ...
      3. Call run_chat() with chapter text as the single user message. run_chat
          ...
          normalizes whitespace (em dashes are preserved under the advisory policy).
      4. Write the normalized Markdown to summaries/chapters/<stem>.md
    ...
    try:
        ...
        # JSON. sanitize_chat() runs inside run_chat to normalize whitespace
        # (em dashes are preserved under the Open-Write advisory policy).
...
async def generate_scene_summary(request: GenerateSceneSummaryRequest):
    ...
    # Body of the summary -- one AI call, plain-text run_chat so the output
    # normalizer (sanitize_chat) runs automatically.
