...
def _parse_profile_markdown(raw: str, filename: str, profile_type: str) -> Profile:
    ...
    parts = raw.split("---", 2)
    if len(parts) < 3:
        # Tolerant fallback: file has no frontmatter. Create default metadata
        # and treat the whole content as the profile body under an Overview heading.
        meta = {
            "type": profile_type,
            "profile_id": filename.replace(".md", "").lower().replace(" ", "_"),
