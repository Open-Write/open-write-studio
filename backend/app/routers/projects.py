...
# =====================================================
# This file handles everything related to Open-Write writing projects:
...
VALID_STORY_TYPES = ("novel", "novella", "novelette", "short_story", "serial_fiction", "screenplay", "tv_pilot")
...
STARTER_FILES = {
    ...
    # Style guide -- pre-populated with the em-dash advisory (Open-Write policy)
    "notes/style-guide.md": (
        "# Style Guide\n\n"
        "## Punctuation Rules\n\n"
        "- Use em dashes sparingly -- aim for under two per page. The "
        "naturalism critic and the em_dash lint flag higher density as an "
        "AI-prose tell. Prefer commas, parentheses, or colons for most breaks.\n\n"
        "## Voice and Tone\n\n"
...
# (defaults to ~/Documents/Open-Write). The writer never has to pick a folder
...
class CreateProjectRequest(BaseModel):
    ...
    # the configured vault root (default ~/Documents/Open-Write) using a
    ...
    if not os.path.exists(project_file):
        raise HTTPException(
            ...
            detail=f"No project.json found in: {folder_path}\n"
                   "This folder doesn't appear to be an Open-Write project."
        )
...
async def create_project(request: CreateProjectRequest):
    """
    Creates a new Open-Write writing project.
    ...
    # project under the configured vault (default ~/Documents/Open-Write)
    ...
    if explicit_folder:
        ...
        if os.path.exists(os.path.join(folder, "project.json")):
            raise HTTPException(
                ...
                detail="An Open-Write project already exists in this folder. "
...
async def open_project(request: OpenProjectRequest):
    """
    Opens an existing Open-Write project by reading its project.json.

    The frontend sends the folder path the user selected.
    We validate it's a real Open-Write project folder and return its info.
    """
...
#   - "unknown" -> show a friendly "this doesn't look like Open-Write" error
#
...
def _migrate_bible_characters(bible_chars_dir: str, profiles_chars_dir: str) -> int:
    """
    Migrate character files from the old bible/03_characters/ layout to the
    structured profiles/characters/ format with YAML frontmatter.

    Converts freeform narrative markdown into the structured format the profile
    parser expects (frontmatter + standard section headings).
    """
    import re as _re
    import yaml as _yaml

    migrated = 0
    for fname in os.listdir(bible_chars_dir):
        if not fname.endswith(".md") or fname.startswith("_"):
            continue

        src_path = os.path.join(bible_chars_dir, fname)
        dst_path = os.path.join(profiles_chars_dir, fname)
        ...
        # Extract character name from the heading or filename
        name_match = _re.search(r"# .*(?:Character Profile:?\s*)(.+)", content, _re.IGNORECASE)
        if name_match:
            char_name = name_match.group(1).strip()
        else:
            char_name = fname.replace(".md", "").replace("_", " ").title()

        # Build structured profile with frontmatter
        frontmatter = {
            "type": "character",
            "profile_id": fname.replace(".md", "").lower().replace(" ", "_"),
            "name": char_name,
            "role": "",
            "status": "active",
            "tags": [],
        ...
        # Build the profile body with standard sections
        # Put the original content under an Overview section
        body = content
        # Remove the original heading line from the body
        body = _re.sub(r"^# .*\n+", "", body)

        structured = "---\n" + _yaml.dump(frontmatter, default_flow_style=False, allow_unicode=True) + "---\n\n"
        structured += "# Overview\n\n"
        structured += body.strip()
        structured += "\n"

        try:
            open(dst_path, "w", encoding="utf-8").write(structured)
            migrated += 1
        except OSError:
            pass
...
async def inspect_folder(path: str):
    """
    Look at a folder and report whether it contains an Open-Write project,
    ...
    # ── Open-Write methodology project auto-detection ─────────────────────
    # Old Open-Write projects (created by the agentic IDE workflow) use the
    # methodology folder layout (bible/, manuscript/, state/) but don't have
    # a project.json. Detect this layout and auto-create one so the app can
    # open the project seamlessly.
    open_write_markers = ["bible", "manuscript", "state"]
    existing_dirs = set()
    try:
    ...
    if all(m in existing_dirs for m in open_write_markers):
        # This is an Open-Write methodology project. Try to get the title
        # from state/project_state.json if it exists.
        auto_title = os.path.basename(path)
        state_file = os.path.join(path, "state", "project_state.json")
        if os.path.isfile(state_file):
            try:
                with open(state_file, "r", encoding="utf-8-sig") as f:
                    sdata = json.load(f)
                auto_title = sdata.get("project", auto_title)
            except (json.JSONDecodeError, OSError):
                pass

        # Auto-create profiles/ folders if they don't exist
        for folder in ["characters", "relationships", "locations", "lore", "chapters", "scenes"]:
            os.makedirs(os.path.join(path, "profiles", folder), exist_ok=True)

        # Migrate characters from bible/03_characters/ if profiles/characters is empty
        bible_chars = os.path.join(path, "bible", "03_characters")
        profiles_chars = os.path.join(path, "profiles", "characters")
        if os.path.isdir(bible_chars) and os.path.isdir(profiles_chars):
            existing_profiles = [f for f in os.listdir(profiles_chars) if f.endswith(".md") and f != "_template.md"]
            if not existing_profiles:
                try:
                    migrated = _migrate_bible_characters(bible_chars, profiles_chars)
                    if migrated:
                        print(f"[MIGRATE] Migrated {migrated} character profiles from bible/ to profiles/")
                except Exception as e:
                    print(f"[MIGRATE] Failed to migrate characters: {e}")

        # Also ensure notes/ and exports/ exist
        for folder in ["notes", "exports"]:
            os.makedirs(os.path.join(path, folder), exist_ok=True)

        # Auto-create project.json so the app can work with this project
        ...
        if not os.path.isfile(project_file):
            try:
                with open(project_file, "w", encoding="utf-8") as f:
                    json.dump(auto_pdata, f, indent=2)
            except OSError:
                pass
    ...
    if not os.path.isfile(project_json_path):
        raise HTTPException(
            ...
            detail="project.json not found -- can't apply a template to a "
                   "folder that isn't an Open-Write project."
        )
