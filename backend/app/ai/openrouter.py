...
async def test_connection(api_key: str, base_url: str = OPENROUTER_BASE) -> dict:
    """
    Verify that the API key is valid by fetching the model list from the given
    provider's base_url. Returns {"ok": True, "model_count": N} on success,
    ...
    try:
        ...
        return {"ok": False, "error": f"Provider returned HTTP {e.response.status_code}."}
        ...
        async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as client:
            response = await client.post(
                ...
                headers={
                    ...
                    # OpenRouter recommends these headers for tracking; other
...
async def run_chat(
    ...
    `include_reasoning` changes the RETURN SHAPE (deliberately, so existing
    call sites stay untouched):
      - False (default): returns the reply string, exactly as before.
      - True: asks for the model's reasoning trace and returns a
        (reply, reasoning) tuple. `reasoning` is None when the model didn't
        emit a trace despite the request.
    """
    ...
    # Ask for the reasoning trace. "medium" effort is a balanced default;
    # the trace comes back on message.reasoning in the response.
