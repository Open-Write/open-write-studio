...
# ===========================================================================
# Each project has its own SQLite database at `<project>/.open-write/app.db`.
...
#   - .open-write/ is already documented as cache; if the file is deleted
...
def get_db_path(project_path: str | Path) -> Path:
    """
    Return the per-project SQLite path: `<project>/.open-write/app.db`.

    The `.open-write/` directory is the documented cache home (see CLAUDE.md
    ...
    database.
    """
    return Path(project_path) / ".open-write" / "app.db"
...
async def open_db(project_path: str | Path) -> AsyncIterator[aiosqlite.Connection]:
    ...
    The first call against a new project creates `.open-write/` and the
