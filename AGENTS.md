...
- **Phase ST (Screenplay/TV pipeline support)** — DONE + tested. The pipeline now adapts to project type (novel/screenplay/TV). `ProjectTypeConfig` captures type-specific parameters (rule dir, unit label, paths, assembly strategy). Type-aware system prompts load from `screenplay_template/.kilo/` or `tv_template/.kilo/` with novel fallback. Manifest builder detects scene/episode headings. Project creation creates type-appropriate folders (`script/scenes/` for screenplay, `scripts/scenes/` for TV). Frontend shows type-aware labels ("Screenwriter", "Scene Critics", etc.). All 39 existing tests pass — backward compatible.
