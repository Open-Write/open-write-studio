...
**Every chapter runs through every critic — no exceptions, no batching, no shortcuts.**

**Every critic is blinded** — reads only the chapter text + its specific rubric. Critics do NOT read the architect plan, writer's intentions, or other critic outputs. This prevents rubber-stamp consensus.

**Every critic output must contain located findings** — quoted text with position. A bare PASS with zero evidence is a failed review, not a clean chapter.

---

## Cardinal Rules

1. **Every chapter gets identical rigor.** No batch mode, no fast path, no abbreviation for later chapters.
...
4. **Full-manuscript reviews read the entire manuscript.** No sampling.
5. **Critics are blinded.** Each reads only the chapter + its rubric. No cross-pollination.
6. **Located findings are mandatory.** Every flagged issue must cite quoted text + position.
7. **Revise-then-recheck.** Each finding marked resolved with before/after, then the chapter re-linted.
...
**Writes:** `critic_outputs/chapter_N_plan.md`
...
**Does NOT write prose.** The plan is the quality gate before generation.
**Verification:** Plan file must exist on disk before prose-writer runs.

### Prose-Writer
**Role:** Executes the architect's plan in prose.
**Reads:** Format rules (every chapter), architect plan, character profiles, prior chapter tail, voice spec.
**Writes:** `manuscript/chapters/NNN_*.md`
**Key discipline:** Rendered interiority, prose distance modulation, subtext not statement, AI tic avoidance.

### Show-Don't-Tell Critic (BLINDED)
**Role:** Mechanical enforcement of format rules.
**Reads:** Chapter file + `bible/07_format_rules.md` ONLY. Does NOT read architect plan or other critics.
**Categories:** Emotional state names, adverbs in dialogue tags, interiority through telling, characters saying what they mean, over-described action, **invisible information** (durations, off-screen knowledge, historical interiority), pure summary.
**Output:** `critic_outputs/chapter_N_show_dont_tell.md`
**Must include:** chapter_hash, located findings (quoted text + line number). Bare PASS fails.

### Voice Critic (BLINDED)
**Role:** Per-character voice consistency review. One call per character in the scene.
**Reads:** Chapter file + one character's voice profile ONLY.
...
**Must include:** chapter_hash, located findings.
...
**Reads:** Chapter file + chapter's palette from outline ONLY.
**Bar:** "Palette lands" not just "palette present." Would a contest reader feel this?
**Evaluates:** Palette achievement, emotional tension, restraint, specificity, rendering depth (scene vs summary).
**Output:** `critic_outputs/chapter_N_palette.md`
**Must include:** chapter_hash, located findings (quoted passages).

### Continuity Critic (BLINDED)
**Role:** State/timeline/callback/audience-state review with deep verification.
**Reads:** Chapter file + state files ONLY. Does NOT read other critics.
...
**Deep verification:** Decomposes chapter into narrative claims, cross-references sub-assumptions against state files, assesses severity of any contradictions.
**Output:** `critic_outputs/chapter_N_continuity.md`
**Must include:** chapter_hash, located findings, deep verification analysis.

### Naturalism Critic (BLINDED)
**Role:** AI-tell detection and naturalism review.
**Reads:** Chapter file ONLY. Does NOT read architect plan, other critics, or state files.
**Detects:** Em-dash overuse, triplet closing patterns, inhuman style consistency, sentence pattern overuse, dialogue tag patterns, thematic restatement, interiority tics, negative-construction density, cross-chapter refrain.
**Output:** `critic_outputs/chapter_N_naturalism.md`
**Must include:** chapter_hash, located findings.
**Runs on every chapter** — not just when context allows. Naturalism catches micro-patterns that make writing read as AI-generated.
...
**Focus:** Prose density, AI tics, generic interiority. Interiority that does work is preserved.
**Output:** Overwrites chapter file + `critic_outputs/chapter_N_cuts.md`

### Editorial Evaluation (BLINDED from other critics)
**Role:** Three-person panel (Vasquez, Webb, Marsh) reviewing finished chapters.
**Reads:** Chapter file + bible ONLY. Does NOT read other critic outputs — evaluates independently.
**Includes:** Structural assessment (causal logic, arc progress, character architecture, callbacks) before prose evaluation. Rendering depth check (scene vs summary).
**Routes:** Structural issues back to outline/bible, character issues back to character profiles, prose issues to chapter file.
**Output:** `coverage_reports/editorial_report_ch[N].md`
**Must include:** chapter_hash, located findings.

### Adversarial Reader (BLIND — no bible, no critics)
**Role:** Cold coverage without bible access. Reads manuscript only.
**Persona:** Marisol Reyes (novel) / Lara Marsh (screenplay/TV), calibrated against literary character drama.
**Must read:** FULL assembled manuscript. No sampling, no "key chapters."
**Verdict scale:** Would Stop / Would Continue / Engaged (partial drafts) or Rejection / Read with Editorial / Acquisition Recommendation (full manuscripts).
**Output:** `coverage_reports/`
**Must include:** Minimum 5 located weaknesses for any full-manuscript read (even Acquisition Recommendation). Bare APPROVE with zero findings is a FAILED read.
...
**Role:** Cross-chapter review synthesis. Reviews the critics, not the prose.
**Reads:** All critic outputs in `critic_outputs/` for a batch of 2-3 chapters. Previous meta-critic notes in `state/meta_critic_notes.md`.
**Does NOT read:** Manuscript chapters, bible, or state files.
**Output:** `coverage_reports/meta_review_ch[N-M].md` + updated `state/meta_critic_notes.md`
**Runs after:** Every 2-3 chapters have completed the full critic pipeline.
...
## The Deterministic Lint Suite (NEW)

Model-independent content lints that run on every chapter and the assembly. The single-model substitute for cross-model validation.

**What it catches:** duplicate paragraphs, cross-chapter refrain repetition, negative-construction density, banned constructions, round-number padding, pure summary, em-dash overuse, intra-chapter refrain.
...
## The Revise-Then-Recheck Protocol (NEW)
...
WRITE (prose-writer)
  ↓
CRITIQUE (all 5 critics: show, voice, palette, continuity — now with deep verification, naturalism — all BLINDED)
  ↓
...
VERIFY (disk check: all files exist, all have located findings, lints pass)
  ↓
Write resume file → next chapter starts fresh

  After every 2-3 chapters:
  META-CRITIC → synthesis report + refinement notes for next batch
```

Every chapter. No exceptions. No abbreviation.
...
6. **Naturalism must run every chapter.** Skipping it lets AI tells accumulate.
7. **Structural issues must route to outline/bible level.** Patching structural problems at prose level wastes time and doesn't fix the root cause.
8. **Character architecture is structural.** A character without contradiction, blind spot, and interiority method is a structural defect, not a prose issue.
...
12. **Critics need a feedback loop.** Without the meta-critic, critics run in isolation and cannot detect patterns that only emerge across chapters. The meta-critic is the missing self-improvement mechanism.
...
## Pre-Generation Editorial Review

**Testing regime for projects with structural complexity.** Before any generation, run the outline/bible through editorial personas. This prevents generating 50,000+ words against a flawed structure — catching structural problems when they're cheap to fix (in the outline) rather than expensive (in the full manuscript).
...
- **Marcus Webb** (development executive, 12 years) — Evaluates marketability and audience appeal.

### Protocol

1. Present each outline option to each persona independently
2. Each persona produces coverage (verdict + specific issues)
3. Synthesize feedback across personas
4. Revise the outline
5. Re-present to all personas
6. Iterate until all return positive verdicts
7. Lock the outline
