...
**When to use:** Sections that lack specificity, scenes that tell instead of show, narrative gaps that need filling.

**What it does:** Identifies weaknesses in the current draft, generates supporting detail, fills reasoning gaps, strengthens underdeveloped sections with concrete scene work.

**Procedure:**
1. Identify passages that are thin, generic, or unsupported
2. For each: what specific sensory detail, action, or dialogue would ground this?
...
**Example:** "The market was busy" → render three specific vendors, the smell of fish, a child running between legs.

#### 2. Combination

**When to use:** Multiple competing approaches exist for a scene/chapter/structure, each with different strengths.
...
**What it does:** Fixes internal consistency. Resolves contradictions between scenes, ensures character behavior tracks across chapters, improves narrative flow. The "plumbing" revision — invisible when done right.

**Procedure:**
1. Identify the contradiction or gap
2. Trace it to its source (which scene/chapter introduced it?)
3. Fix at the source, not at the symptom
4. Verify: does the fix create new contradictions downstream?

**Example:** Character A is afraid of water in Ch3 but swims in Ch7 → fix: either remove the fear in Ch3 or add a justification arc in Ch5-6.
