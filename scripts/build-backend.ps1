...
#      dist/open-write-backend.exe with the FastAPI server inside.
...
Write-Host "    backend/ -> dist/open-write-backend.exe"
Write-Host ""
...
$tripleName = "open-write-backend-x86_64-pc-windows-msvc.exe"
$source     = Join-Path $backendDir "dist\open-write-backend.exe"
